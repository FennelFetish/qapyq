...
class StatsLayout(QtWidgets.QVBoxLayout):
    ...
    def _buildFilesGroup(self, name: str):
        ...
        self.cboCombineMode.setItemData(0, f"List files which have at least one of the selected {name}", Qt.ItemDataRole.ToolTipRole)
        self.cboCombineMode.addItem("One", CombineModeExclusive)
        self.cboCombineMode.setItemData(1, f"List files which have exactly one of the selected {name}", Qt.ItemDataRole.ToolTipRole)
        self.cboCombineMode.addItem("Multiple", CombineModeMultiple)
        self.cboCombineMode.setItemData(2, f"List files which have more than one of the selected {name}", Qt.ItemDataRole.ToolTipRole)
        self.cboCombineMode.addItem("All (Intersection)", CombineModeIntersection)
        self.cboCombineMode.setItemData(3, f"List files which have all selected {name}", Qt.ItemDataRole.ToolTipRole)
        ...
        self.chkFilesNegate.setToolTip(f"List only files that would be hidden with the selected {name} and listing mode")
